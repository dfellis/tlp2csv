...
var tlp2csv = {
	...
	load: function tlp2csvLoad(data) {
		...
		data.toString().split('\n').forEach(function(line) {
			...
			} else if(state == 'data' && line != "") {
				line = line.replace(/[\t]/g, ",");
				line = line.replace(/[ ]+/g, ",");
				line.split(',').forEach(function(value, index) {
					tlp2csv.data[cols[index]].push(parseFloat(value));
				});
			}
		});
	}
...
// If used in a CommonJS (Node.js) environment
if(module && module.exports) { module.exports = tlp2csv; }
